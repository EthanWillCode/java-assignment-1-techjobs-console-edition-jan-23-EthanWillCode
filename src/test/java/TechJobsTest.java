...
public class TechJobsTest {
    ...
    public void testPrintJobs() throws IOException {
        String input = "0\n2\nBuzzbold\nx";
        String output = runProgramWithInput(input).replaceAll("\r", ""); ///*****
        String expected = getFileContents("src/test/resources/testPrintJobs.txt").replaceAll("\r", ""); ///****
    ...
    public void testFindByValue() throws IOException {
        String input = "0\n0\nRuby\nx";
        String output = runProgramWithInput(input).replaceAll("\r", ""); ///*****;
        String expected = getFileContents("src/test/resources/testFindByValue.txt").replaceAll("\r", ""); ///*****;
        assertEquals(expected, output);
    }
    ...
    public void testCaseInsensitiveSearch() throws IOException {
        String input = "0\n3\nnew YORk\nx";
        String output = runProgramWithInput(input).replaceAll("\r", ""); ///*****;
        String expected = getFileContents("src/test/resources/testCaseInsensitiveSearch.txt").replaceAll("\r", ""); ///*****;
        assertEquals(expected, output);
    }
